...
Модуль `generators` содержит функции для работы с банковскими транзакциями и генерации номеров банковских карт. Он находится в папке `src` и включает следующие функции:

### Функции модуля

#### 1. `filter_by_currency(transactions_list, currency)`
Фильтрует список транзакций по заданной валюте.

**Параметры:**
- `transactions_list (list)`: Список транзакций.
- `currency (str)`: Код валюты (например, "USD", "RUB").

**Возвращает:**
- Список транзакций, где валюта операции соответствует заданной.
...
print(filtered)
# Вывод: [{'operationAmount': {'currency': {'code': 'USD'}}}, {'operationAmount': {'currency': {'code': 'USD'}}}]
```

---

#### 2. `transaction_descriptions(transactions_list)`
...
**Возвращает:**
- Список отформатированных номеров карт.

**Пример использования:**
```python
cards = card_number_generator(1000, 1005)
print(cards)
# Вывод: ['0000 0000 0000 1000', '0000 0000 0000 1001', ..., '0000 0000 0000 1005']
```
...
Тесты для модуля `generators` находятся в файле `test_generators.py`. Они проверяют корректность работы всех функций.
